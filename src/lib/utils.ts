...
export function generateId(): string {
  ...
  // randomUUID exists only in a secure context (a plain-http preview has no
  // randomUUID), but getRandomValues is everywhere. These ids become primary
  // keys and replayed queue ops, so they come from the CSPRNG, never Math.random.
