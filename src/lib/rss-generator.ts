...
function cdata(value: string | undefined): string {
  // A CDATA section ends at the first "]]>", so a title or excerpt containing one would
  // terminate the section early and produce a document no reader can parse. Splitting it
  // across two sections is the standard fix and is invisible to the consumer.
