...
/**
 * US-593: the quantity maths behind RecipeAddToGroceryModal's "Add to grocery".
 *
 * Extracted out of the component because vitest only collects `src/**` and the
 * repo has no React Native render preset — this is the logic that was actually
 * broken, so this is where the coverage belongs.
 */
describe('requiredAfterStock', () => {
  it('returns the full amount when nothing is stocked', () => {
    expect(requiredAfterStock(2, 'lb', 0, '')).toBe(2);
    expect(requiredAfterStock(2, 'lb', -1, 'lb')).toBe(2);
  });

  it('preserves sub-1 quantities (was: Math.max(1, …) rounded 0.5 tsp up to 1)', () => {
    expect(requiredAfterStock(0.5, 'tsp', 0, '')).toBe(0.5);
    expect(requiredAfterStock(0.25, 'tsp', 0, '')).toBe(0.25);
  });

  it('subtracts stock in the same unit', () => {
    expect(requiredAfterStock(3, 'lb', 1, 'lb')).toBe(2);
  });

  it('converts stock across compatible units before subtracting', () => {
    // 8 oz of a 1 lb requirement leaves half a pound.
    expect(requiredAfterStock(1, 'lb', 8, 'oz')).toBeCloseTo(0.5, 3);
    // 2 cups stocked against a 1 quart requirement leaves half a quart.
    expect(requiredAfterStock(1, 'qt', 2, 'cup')).toBeCloseTo(0.5, 2);
  });

  it('buys the full amount when the units are incomparable (was: 2 cup − 1 bag = 1)', () => {
    expect(requiredAfterStock(2, 'cup', 1, 'bag')).toBe(2);
    expect(requiredAfterStock(2, 'cup', 5, 'jar')).toBe(2);
  });

  it('treats singular and plural of the same unit as comparable (US-583)', () => {
    expect(requiredAfterStock(3, 'can', 1, 'cans')).toBe(2);
    expect(requiredAfterStock(3, 'bags', 1, 'bag')).toBe(2);
  });

  it('never returns a negative amount when stock exceeds the requirement', () => {
    expect(requiredAfterStock(1, 'lb', 5, 'lb')).toBe(0);
  });

  it('subtracts bare numbers when neither side carries a unit', () => {
    expect(requiredAfterStock(4, '', 1, '')).toBe(3);
  });
