...
serve(async (req) => {
  ...
  try {
    ...
    // Per-user rate limit (cost/DoS protection) before any AI work.
    const limited = await enforceRateLimit(supabaseClient, user.id, 'ai-meal-plan', corsHeaders);
    ...
    if (kidsError || !kids || kids.length === 0) {
      return new Response(
        JSON.stringify({ error: 'Failed to fetch kids' }),
