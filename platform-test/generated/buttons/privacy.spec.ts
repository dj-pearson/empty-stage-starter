...
test.describe('Button Interactions: /privacy', () => {
  test('Test all interactive buttons on /privacy', async ({ page }) => {
    const formFiller = new FormFiller(page, config);
    const locator = new SmartLocator(page);

    // Step 1: Navigate to http://localhost:8080/privacy
    await page.goto('http://localhost:8080/privacy', { waitUntil: 'networkidle' });
