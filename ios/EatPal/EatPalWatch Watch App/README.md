...
**The targets are now wired up in `project.yml`** (`EatPalWatch` and
`EatPalWatchComplications`) and generated by `xcodegen generate` like every
other target — no manual Xcode steps needed. The "Xcode steps" section below
is kept only as a description of what that config does. Bundle ids:
`com.eatpal.app.watchkitapp` (app) and
`com.eatpal.app.watchkitapp.complications` (complications); both carry the
`group.com.eatpal.app` App Group so the complication reads the cached snapshot.
